...
import { baseTheme, highContrastTheme} from "../components/drinks/drinksTheme";
import { CartPanel } from "../components/cartPanel"
import CreditRepaymentManager from '../components/CreditRepaymentManager';
import AppHeader from '../components/AppHeader';
import InventoryGrid from '../components/DrinksInventoryGrid'
...
export default function Drinks() {
  ...
  const cacheInventory = (branchId, data) => {
    if (!branchId) return;
    try {
      localStorage.setItem(`drinks_inventory_${branchId}`, JSON.stringify({ data, ts: Date.now() }));
    } catch {}
  };
  const getCachedInventory = (branchId) => {
    try {
      const raw = localStorage.getItem(`drinks_inventory_${branchId}`);
      if (!raw) return null;
  ...
  const fetchInventory = useCallback(
    debounce(async (branchId) => {
      ...
      setLoading(true);
      try {
        const res = await clientPOS.get(`/api/branches/${branchId}/inventory?type=drinks`);
...
const downloadReceipt = async () => {
  ...
  const watermarkText = "Drinks POS"; // <-- extra spacing
  ...
  doc.text("King Of The Grill Drinks POS Receipt", 105, 20, { align: "center" });
...
// put this ABOVE export default Gas
  ...
  return (
    <ThemeProvider theme={theme}>
      <Helmet>
        <title>Drinks: King of the grill</title>
        <meta name="theme-color" content="#006494" />
        <link rel="icon" href="/assets/images/drinks.png" type="image/png" />
      </Helmet>
      <ImmersiveScene />
      <style jsx>{`
        :global(.water‑sync) {
          --mui-primary: var(--scene-color);
          --mui-primary-rgb: var(--scene-color-rgb, 0,163,224);
          background: linear-gradient(
            145deg,
            rgba(var(--mui-primary-rgb), calc(0.08 + var(--scene-shimmer,0.5)*0.25)),
          ...
          {!isAuthenticated ? (
            ...
            <>
              <AppHeader
                ...
                setRepayOpen={handleCreditModalOpen}  // Adjust if needed for your credit modal
                inventory={inventory}
                cart={cart}
              ...
              <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
                <Typography variant="h5" sx={{ fontWeight: 800, color: '#222831' }}>
                  Inventory
                </Typography>
                <Button variant="outlined" onClick={() => user?.branch?.id && fetchInventory(user.branch.id)} sx={{ borderRadius: 10 }}>
                  Refresh
                </Button>
              </Box>
